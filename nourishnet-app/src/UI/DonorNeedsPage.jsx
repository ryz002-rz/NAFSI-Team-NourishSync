...
function DonorNeedsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(l => (l.name || '').toLowerCase().includes(q) || (l.address?.city || '').toLowerCase().includes(q) || (l.address?.zip || '').includes(q));
    }
  ...
  return (
    <div className="fd-root">
      ...
      <h1 className="fd-title">{t('ui.sortByNeeds')}</h1>
      <p className="fd-count">{filtered.length} {t('ui.locations')}</p>
      <FilterBar allFoodTypes={ALL_FOOD_TYPES} onFilter={setFilters} totalCount={sorted.length} filteredCount={filtered.length} />
      <div className="fd-list">
        {filtered.map(loc => (<NeedCard key={loc.id} loc={loc} t={t} navigate={navigate} />))}
    ...
    <div className="fd-card">
      <div className="fd-card-top">
        <div>
          ...
          {loc.insecurityIndex >= 4 && <span style={{ fontSize: 11, color: '#ef4444', fontWeight: 600 }}>🔴 High Need (Level {loc.insecurityIndex})</span>}
        </div>
      ...
      <div className={`fd-card-expand${expanded ? ' fd-card-expand--open' : ''}`}>
        <div className="fd-card-expand-inner">
          ...
          <button className="fd-card-map-btn" onClick={() => navigate(`/donor/map?loc=${loc.id}`)}>🗺️ {t('ui.showInMap')}</button>
        </div>
      </div>
      <div className="fd-card-bottom">
        <div className="fd-card-tags">{(loc.foodTypes || []).slice(0, 5).map(ft => (<button key={ft} className="fd-card-tag" onClick={() => navigate(`/donor/food/${encodeURIComponent(ft)}`)}>{t(`foodType.${ft.toLowerCase()}`, ft)}</button>))}</div>
